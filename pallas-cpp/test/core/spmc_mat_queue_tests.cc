...
TEST_F(SimpleMatQueueTests, RegisterThenPush) {
    ...
    // Now read frames - consumer should see all frames pushed after registration
    for (size_t i = 0; i < frames_.size(); ++i) {
        cv::Mat popped;
...
TEST_F(SimpleMatQueueTests, PushThenRegister) {
    ...
    cv::Mat popped;
    EXPECT_FALSE(queue_.try_pop(popped)) << "Consumer incorrectly read a frame pushed before registration";
...
TEST_F(SimpleMatQueueTests, TwoConsumers) {
    ...
    for (size_t i = 2; i < frames_.size(); ++i) {
        ...
        if (pop_result) {
            ...
            EXPECT_DOUBLE_EQ(0.0, min);
            EXPECT_DOUBLE_EQ(static_cast<double>(i), max) 
                << "Frame " << i << " has incorrect content for second consumer";
        }
    }
    
    // Verify there are no more frames to read
    cv::Mat extra;
    EXPECT_FALSE(queue_.try_pop(extra)) << "First consumer got unexpected extra frame";
    EXPECT_FALSE(consumer2_queue.try_pop(extra)) << "Second consumer got unexpected extra frame";
