...
namespace pallas {
class SegmentAnythingTests : public testing::Test {
   protected:
    void SetUp() override {}

    void TearDown() override {}
};
TEST_F(SegmentAnythingTests, Beep) {
    // Precondition.
    const std::filesystem::path assets_path = "../assets/";
    const std::filesystem::path encoder_path =
        assets_path / "sam2.1_tiny_preprocess.onnx";
    const std::filesystem::path decoder_path = assets_path / "sam2.1_tiny.onnx";
    const std::string device = "cpu";

    SegmentAnything sam;
    ASSERT_TRUE(sam.loadModel(encoder_path, decoder_path,
                              std::thread::hardware_concurrency(), device));
...
// DEFINE_string(encoder, "sam2.1_tiny/sam2.1_tiny_preprocess.onnx", "Path to
// the encoder model"); DEFINE_string(decoder, "sam2.1_tiny/sam2.1_tiny.onnx",
// "Path to the decoder model"); DEFINE_string(image,
// "david-tomaseti-Vw2HZQ1FGjU-unsplash.jpg", "Path to the image");
...
//   if(FLAGS_h){
//     std::cout<<"Example: ./build/sam_cpp_test
//     -encoder=\"sam2.1_tiny/sam2.1_tiny_preprocess.onnx\" "
...
//   bool successLoadModel = sam.loadModel(FLAGS_encoder, FLAGS_decoder,
//   std::thread::hardware_concurrency(), FLAGS_device); if(!successLoadModel){
...
//   std::chrono::steady_clock::now(); bool successPreprocessImage =
//   sam.preprocessImage(image); std::chrono::steady_clock::time_point end =
//   std::chrono::steady_clock::now(); std::cout << "sec = " <<
//   (std::chrono::duration_cast<std::chrono::microseconds>(end -
//   begin).count()) / 1000000.0 <<std::endl; if(!successPreprocessImage){
...
//     mask = sam.getMaskBatch(inputPointValues, inputLabelValues, batchNum,
//     imageSize); cv::imwrite("mask_box_batch.png", mask);
...
//   rects.push_back(rect1);
//   sam.setRectsLabels(rects, &inputPointValues, &inputLabelValues);
//   mask = sam.getMask(inputPointValues, inputLabelValues, imageSize,
//   previousMaskIdx, isNextGetMask); previousMaskIdx++;
...
//   std::cout << "sec = " <<
//   (std::chrono::duration_cast<std::chrono::microseconds>(end -
//   begin).count()) / 1000000.0 <<std::endl; return 0;
// }
